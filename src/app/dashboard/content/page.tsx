          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
            <div>
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {personas.map(p => (
                    <button key={p.id} onClick={() => setSelectedPersona(p)} style={{ padding: '10px 14px', borderRadius: 8, border: selectedPersona?.id === p.id ? '1px solid var(--electric)' : '1px solid rgba(255,255,255,0.08)', background: selectedPersona?.id === p.id ? 'rgba(0,245,255,0.06)' : 'transparent', color: selectedPersona?.id === p.id ? 'var(--electric)' : 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: 14, textAlign: 'left', transition: 'all 0.15s' }}>{p.name}</button>
                  ))}
                </div>
              </div>
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {contentTypes.map(ct => (
                    <button key={ct.key} onClick={() => { setContentType(ct); setOutput('') }} style={{ padding: '10px 14px', borderRadius: 8, border: contentType.key === ct.key ? '1px solid var(--electric)' : '1px solid rgba(255,255,255,0.08)', background: contentType.key === ct.key ? 'rgba(0,245,255,0.06)' : 'transparent', color: contentType.key === ct.key ? 'var(--electric)' : 'rgba(255,255,255,0.5)', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}>
            ...
            <div>
              ...
              <div style={{ minHeight: 320, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: 16, fontSize: 14, color: output ? 'var(--cream)' : 'rgba(255,255,255,0.2)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
                {loading ? <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--electric)', fontSize: 13 }}><RefreshCw size={16} style={{ animation: 'spin 1s linear infinite' }} /> Writing in {selectedPersona?.name}'s voice...</div> : output || `Your ${contentType.label.toLowerCase()} will appear here...`}
              </div>
            </div>
          </div>
        )}
