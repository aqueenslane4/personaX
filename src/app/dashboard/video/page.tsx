  ...
  const generateVideo = async () => {
    ...
    try {
      const res = await fetch('/api/video', {
        ...
        body: JSON.stringify({ imageUrl: uploadedImage, motionPrompt: customPrompt || motion.value }),
      })
      const data = await res.json()
      ...
      <div style={{ maxWidth: 1000, margin: '28px auto', padding: '0 24px' }}>
        {outOfCredits && (
          <div style={{ background: 'linear-gradient(135deg, rgba(255,77,0,0.08), rgba(123,47,255,0.08))', border: '1px solid rgba(255,77,0,0.25)', borderRadius: 12, padding: '24px', marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
            ...
            {!isPro && <Link href="/pricing?plan=pro" className="btn-primary" style={{ padding: '11px 24px', fontSize: 13, whiteSpace: 'nowrap' }}>Upgrade to Pro <ArrowRight size={15} /></Link>}
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 24 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div>
              ...
              <div onClick={() => fileRef.current?.click()} style={{ aspectRatio: '1', borderRadius: 10, border: uploadedImage ? '1px solid rgba(0,245,255,0.3)' : '2px dashed rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.02)', cursor: 'pointer', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8 }}>
                {uploadedImage ? <img src={uploadedImage} alt="Source" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <><Upload size={28} color="rgba(255,255,255,0.2)" /><span style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', textAlign: 'center' }}>Upload your AI persona photo</span></>}
              </div>
              <input ref={fileRef} type="file" accept="image/*" onChange={handleImageUpload} style={{ display: 'none' }} />
              {uploadedImage && <button onClick={() => fileRef.current?.click()} style={{ width: '100%', marginTop: 8, padding: '7px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.08)', background: 'transparent', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: 12 }}>Change image</button>}
            </div>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                {motionStyles.map(m => <button key={m.label} onClick={() => setMotion(m)} style={{ padding: '9px 12px', borderRadius: 6, border: motion.label === m.label ? '1px solid var(--electric)' : '1px solid rgba(255,255,255,0.07)', background: motion.label === m.label ? 'rgba(0,245,255,0.06)' : 'transparent', color: motion.label === m.label ? 'var(--electric)' : 'rgba(255,255,255,0.45)', cursor: 'pointer', fontSize: 12, textAlign: 'left' }}>{m.label}</button>)}
              </div>
            </div>
            <div>
            ...
            <button className="btn-primary" onClick={generateVideo} disabled={isGenerating || !uploadedImage || outOfCredits} style={{ width: '100%', justifyContent: 'center', opacity: isGenerating || !uploadedImage || outOfCredits ? 0.5 : 1 }}>
              {isGenerating ? <><RefreshCw size={15} style={{ animation: 'spin 1s linear infinite' }} /> {status === 'polling' ? 'Processing...' : 'Starting...'}</> : outOfCredits ? <><Lock size={15} /> No Credits Left</> : <><Sparkles size={15} /> Generate Video</>}
            </button>
            <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', textAlign: 'center' }}>~45-90 seconds · {credits} credit{credits !== 1 ? 's' : ''} remaining</p>
          </div>
          ...
          <div>
            <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)', aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 14, marginBottom: 16 }}>
              {status === 'done' && videoUrl ? <video src={videoUrl} autoPlay loop muted playsInline style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : isGenerating ? <><RefreshCw size={44} color="var(--electric)" style={{ animation: 'spin 2s linear infinite' }} /><div style={{ textAlign: 'center' }}><div style={{ fontSize: 14, color: 'var(--electric)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 6 }}>{status === 'polling' ? 'Animating your persona...' : 'Starting generation...'}</div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.25)' }}>This takes 45-90 seconds</div></div></> : <><Video size={52} color="rgba(255,255,255,0.08)" /><span style={{ fontSize: 13, color: 'rgba(255,255,255,0.2)' }}>Your video will play here</span></>}
            </div>
            {status === 'done' && videoUrl && (
              <div style={{ display: 'flex', gap: 10 }}>
                <a href={videoUrl} download={`personax-${Date.now()}.mp4`} target="_blank" rel="noreferrer" className="btn-primary" style={{ flex: 1, justifyContent: 'center' }}><Download size={15} /> Download MP4</a>
                <button className="btn-ghost" onClick={generateVideo} style={{ flex: 1, justifyContent: 'center' }}><RefreshCw size={15} /> Regenerate</button>
              </div>
            )}
          </div>
        </div>
