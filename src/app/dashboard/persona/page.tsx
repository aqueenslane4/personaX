...
type PersonaForm = { name: string; gender: string; vibe: string; style: string; niche: string; audience: string; bio: string }
...
export default function PersonaBuilder() {
  ...
  const chip = (val: string, selected: boolean, onClick: () => void) => (
    <button key={val} onClick={onClick} style={{ fontSize: 13, padding: '8px 16px', borderRadius: 6, border: selected ? '1px solid var(--electric)' : '1px solid rgba(255,255,255,0.1)', background: selected ? 'rgba(0,245,255,0.08)' : 'transparent', color: selected ? 'var(--electric)' : 'rgba(255,255,255,0.5)', cursor: 'pointer', transition: 'all 0.15s', textAlign: 'left' }}>{val}</button>
  )
    ...
    <main style={{ minHeight: '100vh', background: 'var(--dark)', padding: '0 0 60px' }}>
      ...
      <div style={{ maxWidth: 600, margin: '40px auto', padding: '0 24px' }}>
        <div style={{ display: 'flex', gap: 6, marginBottom: 36 }}>
          {Array.from({ length: totalSteps }).map((_, i) => <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i < step ? 'var(--electric)' : 'rgba(255,255,255,0.08)', transition: 'background 0.3s' }} />)}
        </div>
        {step === 1 && (
          <div>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>{['Female', 'Male', 'Non-binary', 'No preference'].map(g => chip(g, form.gender === g, () => setField('gender', g)))}</div>
            </div>
            <button className="btn-primary" onClick={() => setStep(2)} disabled={!form.name || !form.gender} style={{ width: '100%', justifyContent: 'center', opacity: !form.name || !form.gender ? 0.4 : 1 }}>Next <ArrowRight size={16} /></button>
          </div>
        )}
        {step === 2 && (
          <div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{vibes.map(v => chip(v, form.vibe === v, () => setField('vibe', v)))}</div>
            </div>
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{styles.map(s => chip(s, form.style === s, () => setField('style', s)))}</div>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button className="btn-ghost" onClick={() => setStep(1)} style={{ flex: 1, justifyContent: 'center' }}>Back</button>
              <button className="btn-primary" onClick={() => setStep(3)} disabled={!form.vibe || !form.style} style={{ flex: 2, justifyContent: 'center', opacity: !form.vibe || !form.style ? 0.4 : 1 }}>Next <ArrowRight size={16} /></button>
            </div>
          </div>
        )}
        {step === 3 && (
          <div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{niches.map(n => chip(n, form.niche === n, () => setField('niche', n)))}</div>
            </div>
            <div style={{ marginBottom: 28 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button className="btn-primary" onClick={() => setStep(4)} disabled={!form.niche} style={{ flex: 2, justifyContent: 'center', opacity: !form.niche ? 0.4 : 1 }}>Next <ArrowRight size={16} /></button>
            </div>
          </div>
        )}
        {step === 4 && (
          <div>
            ...
            <div style={{ background: 'rgba(0,245,255,0.04)', border: '1px solid rgba(0,245,255,0.15)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
              {[{ label: 'Name', value: form.name }, { label: 'Gender', value: form.gender }, { label: 'Vibe', value: form.vibe }, { label: 'Style', value: form.style }, { label: 'Niche', value: form.niche }, { label: 'Audience', value: form.audience || 'Not set' }].map((r, i) => (
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.08em', textTransform: 'uppercase', display: 'block', marginBottom: 8 }}>Bio (optional)</label>
